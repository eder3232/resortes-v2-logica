...
export class Spring {
  ...
  generateLocals() {
    for (const [key, value] of this.dataEdges) {
      ...
      const obj = { local: [[k, -k], [-k, k]], tableDOF };
  ...
  solveInternalForces() {
    ...
    const newMap = new Map<string, { u_i: number, u_j: number, k: number, internalForce: number }>()
    for (const [key, value] of this.dataEdges) {
      const u_i = this.solved.u.global[this.utils.dofPointerInDataArray.get(value.DOF.table[0].internal)!][0]
      const u_j = this.solved.u.global[this.utils.dofPointerInDataArray.get(value.DOF.table[1].internal)!][0]
